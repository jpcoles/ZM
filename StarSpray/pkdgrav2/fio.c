...
static int tipsySwitchFile(FIO fio) {
	...
	tio->fp = fopen(fio->fileList.fileInfo[fio->fileList.iFile].pszFilename,"rb"); flock(fileno(tio->fp), LOCK_EX);
...
static FIO tipsyOpen(fioFileList *fileList) {
    ...
    tio->fp = fopen(tio->fio.fileList.fileInfo[0].pszFilename,"rb"); flock(fileno(tio->fp), LOCK_EX);
...
FIO fioTipsyCreate(const char *fileName,int bDouble,int bStandard,
    ...
    tio->fp = fopen(fileName,"wb"); flock(fileno(tio->fp), LOCK_EX);
    tio->fio.fileList.iFile = 0;
    if (tio->fp==NULL) {
	free(tio);
	return NULL;
...
FIO fioTipsyAppend(const char *fileName,int bDouble,int bStandard) {
    ...
    tio->fp = fopen(fileName,"rb+"); flock(fileno(tio->fp), LOCK_EX);
    tio->fio.fileList.iFile = 0;
    if (tio->fp==NULL) {
	free(tio);
	return NULL;
...
FIO fioTipsyCreatePart(const char *fileName,int bAppend,int bDouble,int bStandard,
    ...
    tio->fp = fopen(fileName,bAppend?"rb+":"wb"); flock(fileno(tio->fp), LOCK_EX);
    tio->fio.fileList.iFile = 0;
    if (tio->fp==NULL) {
	free(tio);
	return NULL;
