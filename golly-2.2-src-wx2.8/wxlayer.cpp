...
const int layerbarht = 64;       // height of layer bar
...
// #include "bitmaps/zmgolly_step.xpm"
// #include "bitmaps/zmgolly_play.xpm"
// #include "bitmaps/zmgolly_reset.xpm"
// #include "bitmaps/zmgolly_stop.xpm"
// #include "bitmaps/zmgolly_zoomin.xpm"
// #include "bitmaps/zmgolly_zoomout.xpm"
...
public:
   ...
   int AddButton(int id, int x, const wxString& tip);
...
void LayerBar::OnPaint(wxPaintEvent& WXUNUSED(event))
{
   ...
   if (!showedit) {
      ...
      #endif
      dc.DrawLine(0, r.GetTop(), r.width, r.GetTop());
      dc.SetPen(wxNullPen);
   }
}
...
void LayerBar::OnButton(wxCommandEvent& event)
{
   ...
   switch (id) {
      ...
      case STEP_TOOL:      cmdid = ID_STEP; break;
      case STOP_TOOL:      cmdid = ID_STOP; break;
      ...
      case EX2_EN_TOOL:
         cmdid = ID_STOP;
         mainptr->pendingfiles.Add(wxT("Patterns/Life/Oscillators/traffic-light-hasslers.rle")); 
         break;
...
int LayerBar::AddButton(int id, int x, const wxString& tip)
{
   // create bitmap button
   const int BUTTON_WD = 48;        // nominal width of bitmap buttons
   int wd,ht;
...
void CreateLayerBar(wxWindow* parent)
{
   ...
   x = smallgap;
   layerbarptr->AddButton(EX1_EN_TOOL,   x, _("Example One"));                    
   layerbarptr->AddButton(EX1_DE_TOOL,   x, _("Example One"));                    x += 120+smallgap; 

   layerbarptr->AddButton(EX2_EN_TOOL,   x, _("Example One"));                    
   layerbarptr->AddButton(EX2_DE_TOOL,   x, _("Example One"));                    x += 40+smallgap; 


   x = 1920 - (40 + smallgap);
   layerbarptr->AddButton(HELP_TOOL,  x, _("Help"));                x -= 120+smallgap; 
   layerbarptr->AddButton(CH_TOOL,    x, _("Switch to German"));    x -= 120+smallgap;
   ...
   for (int i = 0; i < MAX_LAYERS; i++) togglebutt[i]->Show(false);
...
void ResizeLayerBar(int wd)
{
   if (layerbarptr) {
      ...
      //layerbarptr->SetPosition(wxPoint(-1, r.y+r.height));
   }
}
...
void UpdateLayerBar(bool active)
{
   if (layerbarptr && showlayer) {
      ...
      layerbarptr->EnableButton(START_TOOL,  active && !inscript);
      layerbarptr->EnableButton(STEP_TOOL,   active && !inscript);
      layerbarptr->EnableButton(STOP_TOOL,   active && !inscript);
      layerbarptr->EnableButton(RESET_TOOL,  active && !inscript);
