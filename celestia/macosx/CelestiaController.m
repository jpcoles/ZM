...
- (void)applicationDidChangeScreenParameters:(NSNotification *) aNotification
{
    if (isFullScreen && aNotification && ([aNotification object] == NSApp))
    {
        ...
        if (!NSEqualSizes(screenRect.size, [[self window] frame].size)) {
			...
			// TODO: this is where you might want to add a button full screen
			// TOOD: find header/framework to import so that first three lines compile
			//create the button
			/*
			...
			[self.view addSubview:button];
			 [button addTarget:self action:@selector(GoToWindowController) 
